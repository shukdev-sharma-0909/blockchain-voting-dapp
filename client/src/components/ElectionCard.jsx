...
const ElectionCard = ({ state }) => {
  ...
  const [status, setStatus] = useState("Voting has not started");
  ...
  const getTime = async () => {
    ...
    try {
      const start = Number(await state.contract.startTime());
      const end = Number(await state.contract.endTime());
      ...
      if (end > 0) {
        const endDate = new Date(end * 1000).toLocaleString();
        setEndTime(endDate);
      }
    } catch (error) {
      console.log(error);
        ...
        {pollEnded ? (
          ...
          >
            ...
            <circle cx="50" cy="50" r="20" fill="#FF3131">
              <animate attributeName="r" values="20;15;20" dur="4s" repeatCount="indefinite" />
            </circle>
          </svg>
        ) : (
          <svg
        ...
        <div className="rounded-[calc(1.5rem-1px)] p-6 w-[100%] bg-white dark:bg-gray-900">
          <div className="  flex flex-col flex-wrap items-center justify-start gap-2">
            ...
            <div className="grid grid-cols-2 font-extralight items-center md:mt-2">
              <p className=" text-gray-500 text-sm whitespace-nowrap ">Current Time:</p>
              <p className=" text- font-semibold text-xs whitespace-nowrap ">{date}</p>
            </div>
            {/* Current time ends */}
            ...
            <div className="flex space-x-20 items-center">
              ...
              <div className="grid grid-rows-2 gap-2 font-extralight items-center">
                <p className=" text-gray-500 text-sm whitespace-nowrap ">Poll Start</p>
                <p className=" text-[#2ABB94] font-semibold text-xs ">{startTime}</p>
              </div>
              {/* Start time ends here */}

              {/* END time starts here */}
              <div className="grid grid-rows-2 gap-2 font-extralight items-center">
                <p className=" text-gray-500 text-sm whitespace-nowrap ">Poll End</p>
                <p className=" text-[#F784AD] font-semibold text-xs ">{endTime}</p>
              </div>
