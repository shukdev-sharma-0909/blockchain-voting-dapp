...
const Candidate = ({ state, handleCase }) => {
  const handleInfo = async (event) => {
    ...
    if (!state?.contract) {
      toast.error("Connect your wallet before registering a candidate.");
      return;
    }
    ...
    const rawParty = partyInput?.value.trim();
    const rawAge = ageInput?.value.trim();
    const rawGender = genderInput?.value.trim();

    if (!rawName || !rawParty || !rawAge || !rawGender) {
    ...
    const candidatesInfo = {
      ...
      party: rawParty.toUpperCase(),
      age,
      gender: handleCase(rawGender),
    };

    try {
      const tx = await state.contract.candidateRegister(
        candidatesInfo.name,
