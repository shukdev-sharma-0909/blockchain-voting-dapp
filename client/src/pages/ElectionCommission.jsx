...
const ElectionCommission = ({ state }) => {
  const handleElection = async (event) => {
    ...
    if (!state?.contract) {
      toast.error("Connect your wallet before starting the election.");
      return;
    }

    const startRaw = document.querySelector("#startT")?.value.trim();
    const endRaw = document.querySelector("#endT")?.value.trim();

    const start = Number(startRaw);
    const end = Number(endRaw);

    if (!Number.isInteger(start) || start < 0 || !Number.isInteger(end) || end <= 0) {
      toast.error("Enter valid start and end times in seconds.");
      return;
    }

    try {
      const tx = await state.contract.voteTime(start, end);
      await tx.wait();
      const date = new Date();
  ...
  const handleEmergency = async () => {
    if (!state?.contract) {
      toast.error("Connect your wallet before declaring an emergency.");
      return;
    }

    try {
      const tx = await state.contract.emergency();
      await tx.wait();
  ...
  const handleResult = async () => {
    if (!state?.contract) {
      toast.error("Connect your wallet before declaring results.");
      return;
    }
